...
(function () {
  ...
  function viewHome(){
    ...
    const stats=`<div class="statrow">
      ...
      <div class="stat"><div class="k">当前体重</div><div class="v">${lastBody?lastBody.fields.weight:'—'}<small> kg</small></div><div class="d">${lastBody&&P.startWeight?('较起始 '+(lastBody.fields.weight-P.startWeight).toFixed(1)+'kg'):'记录后显示'}</div></div>
  ...
  function viewInsight(){
    ...
    const segs=Object.keys(dist).map(t=>{const v=dist[t]/total*100;return `<span style="width:${v}%;background:${palette[t]||'#999'}"></span>`;}).join('');
    ...
    const bp=wkArr.map(o=>o.w);const bmn=Math.min(...bp),bmx=Math.max(...bp);
  ...
  function viewReport(){
    ...
    const body=state.records.filter(r=>r.type==='body').sort((a,b)=>a.date<b.date?1:-1);
    const firstB=body[body.length-1],lastB=body[0];
    ...
    return `<div class="topbar"><div class="brand"><div class="mark">${ART.logo}</div>
      ...
      <div class="detail-body">
        ...
        <div class="statrow">
          <div class="stat"><div class="k">体重变化</div><div class="v">${firstB&&lastB?((lastB.fields.weight-firstB.fields.weight).toFixed(1)):'—'}<small> kg</small></div></div>
          <div class="stat"><div class="k">体脂变化</div><div class="v">${firstB&&lastB?((lastB.fields.fat-firstB.fields.fat).toFixed(1)):'—'}<small> %</small></div></div>
        </div>
  ...
  function handleAct(act,el){
    ...
    switch(act){
      ...
      case 'confirm-add':{
        ...
        const isFirstTrain=r.type==='training'&&!state.records.some(x=>x.type==='training'&&x.id!==r.id);
        if(r.type==='training'&&r.status==='done'){bumpStreak();applyPR(r);}
      ...
      case 'confirm-edit':{
        ...
        if(orig.type==='training'){const wasDone=orig.status==='done';Object.assign(orig,{title:r.title,date:r.date,time:r.time,status:r.status,tags:r.tags,note:r.note,cover:r.cover,fields:r.fields});
          if(r.status==='done'&&!wasDone){bumpStreak();applyPR(orig);}if(r.status!=='done'&&wasDone)bumpStreak();}
      ...
      case 'toggle-status':{
        ...
        if(r.type==='training'){if(r.status==='done'&&was!=='done'){bumpStreak();applyPR(r);}if(r.status!=='done'&&was==='done')bumpStreak();}
  ...
  function bumpStreak(){
    // 真正的"连续训练天数"：按完成训练的日期去重，从今天(或昨天)向前数连续天数
    const dates=state.records.filter(r=>r.type==='training'&&r.status==='done'&&r.date).map(r=>r.date).sort();
    const set=new Set(dates);
    if(!set.size){state.persona.streak=0;return;}
    let start=todayStr();
    if(!set.has(start)){const y=new Date(todayStr());y.setDate(y.getDate()-1);start=localDateStr(y);}
    if(!set.has(start)){state.persona.streak=0;return;}
    let cnt=0;let cur=new Date(start);
    while(set.has(localDateStr(cur))){cnt++;cur.setDate(cur.getDate()-1);if(cnt>3650)break;}
    state.persona.streak=cnt;
    if(cnt>=7)unlock('连续训练 7 天','check','习惯开始扎根。');
    if(cnt>=14)unlock('连续训练 14 天','training','双周节律，稳。');
    if(cnt>=21)unlock('连续训练 21 天','trophy','三周不中断，身体已适应。');
    if(cnt>=30)unlock('连续训练 30 天','record','月度全勤，证据确凿。');
  }
